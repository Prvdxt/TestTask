...
public class VerifyElementIsSearchedWithoutMatchCase extends DriverClass {
	...
	public void searchElementTest() throws Exception {
		...
		System.out.println("Verification of the checkbox for match case is unchecked");		
		boolean checker= callingFunction.verifyTheBoxIsUnchecked();		
		Assert.assertEquals(true, checker, "The Checkbox is not checked");
		...
		boolean check = callingFunction.verifytheResult(textEntered, valueSelected);
		Assert.assertEquals(true, check, "Verify that the search is working Fine");

		System.out.println("Verifying the table data fetched based on text entered.");
		callingFunction.verifytabledataisdisplayedcorrectlyAfterSearch(valueSelected,textEntered);
